...
if [ "$#" -ne 2 ]; then
    echo "Использование: ./run_ml_classifier.sh input_folder output_folder"
    exit 1
fi
...
LOG_FILE="$LOG_FOLDER/ml_classifier.log"
...
echo "Используется ML-классификатор" | tee -a "$LOG_FILE"

"$PYTHON_BIN" "$PROJECT_DIR/app/app_classifier.py" \
    "$INPUT_FOLDER" \
